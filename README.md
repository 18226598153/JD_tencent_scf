...
打京豆的脚本部署流程, 该仓库对使用流程进行了简化, 每个月能打1500左右, 如果你的计算机有安装Docker, 推荐使用[本地部署](#本地部署)方式, 没有就使用[腾讯云函数部署](#腾讯云函数部署), 不计划设计助力池, 如果对你有帮助, 给这个项目点下star, 谢谢.
...
2. 腾讯云日志服务不免费, 余额为0建议往账户里面**充值1元**, 防止欠费被禁用.
3. **非必须, 没有再创建**: 不使用子账户, 保证[服务角色](https://console.cloud.tencent.com/cam/role)有**SCF_QcsRole、SLS_QcsRole**权限.
